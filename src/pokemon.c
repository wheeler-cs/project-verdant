...
s32 CalculateBaseDamage(struct BattlePokemon *attacker, struct BattlePokemon *defender, u32 move, u16 sideStatus, u16 powerOverride, u8 typeOverride, u8 battlerIdAtk, u8 battlerIdDef)
{
    ...
    // Just boost both stats and damage calculation will pick the right stat to use
...
#ifdef PHYS_SPEC_SPLIT
// Make sure weather effects and Flash Fire apply to move typing, not contact type
    // Are effects of weather negated with cloud nine or air lock
    if (WEATHER_HAS_EFFECT2)
    {
        // Rain weakens Fire, boosts Water
...
/**
 * Functions added below used as specials in scripting to get the IVs of the lead party member
 */
u16 GetHPIV (void)
{
...
u8 GetHiddenPowerType(struct Pokemon *mon)
{
    ...
    // Get the bits of the Pokemon's IVs
    type_bits = ((GetMonData(mon, MON_DATA_HP_IV)    & 1) << 0) |
    ...
    // Calculate the type from the bits, and adjust if needed
    type = (15 * type_bits) / 63 + 1;
    if (type >= TYPE_MYSTERY)
        type++;

    // Do some magic bit masking
    type |= 0xC0;
    type &= 0x3F;
